...
def fetch_horse_list_by_year(birth_year: int, max_pages: int = 20) -> pd.DataFrame:
    ...
    for page in range(1, max_pages + 1):
        ...
        for row in rows:
            ...
            sire_age = ""
            if sire_age_link:
                sire_href = sire_age_link.get("href", "")
                tid_match = re.search(r"sire_id=(\d{4})", sire_href)
                if tid_match:
                    sire_age = birth_year - int(tid_match.group(1))

            # [7]: 母の年齢
            dam_age_link = cols[7].find("a")
            dam_age = ""
            if dam_age_link:
                dam_href = dam_age_link.get("href", "")
                tid_match = re.search(r"mare_id=(\d{4})", dam_href)
                if tid_match:
                    dam_age = birth_year - int(tid_match.group(1))

            # [8]: 母父の年齢
            sire_of_dam_age_link = cols[8].find("a")
            sire_of_dam_age = ""
            if sire_of_dam_age_link:
                sire_of_dam_href = sire_of_dam_age_link.get("href", "")
                tid_match = re.search(r"bms_id=(\d{4})", sire_of_dam_href)
                if tid_match:
                    sire_of_dam_age = birth_year - int(tid_match.group(1))
